...
class GigDAOContract:
    ...
    def get_approval_program(self):
        # Đọc mã TEAL từ file và biên dịch nó
        with open("smart_contracts/contract/approval.teal", "r") as f:
            approval_program_teal = f.read()

        # Biên dịch TEAL code và lấy bytecode
        response = self.algod_client.compile(approval_program_teal)

        # Chuyển đổi kết quả trả về từ base64 thành bytes
        approval_program_bytes = base64.b64decode(response['result'])
        return approval_program_bytes

    def get_clear_program(self):
        # Đọc mã TEAL từ file và biên dịch nó
        with open("smart_contracts/contract/clear.teal", "r") as f:
            clear_program_teal = f.read()

        # Biên dịch TEAL code và lấy bytecode
        response = self.algod_client.compile(clear_program_teal)

        # Chuyển đổi kết quả trả về từ base64 thành bytes
        clear_program_bytes = base64.b64decode(response['result'])
        return clear_program_bytes
    ...
    def create_dao(self, address: str, private_key: str, name: str, description: str, voting_quorum: float = 0.5):
        if self.dao_initialized:
            raise Exception("DAO is already initialized.")
        ...
        # Triển khai DAO dưới dạng hợp đồng thông minh lên Algorand
        params = self.algod_client.suggested_params()

        approval_program = self.get_approval_program()  # Cần hàm trả về mã TEAL đã biên dịch
        clear_program = self.get_clear_program()  # Cần hàm trả về mã TEAL đã biên dịch

        # Sử dụng các chỉ số trạng thái của DAO, ví dụ cho các biến toàn cục và cục bộ
        global_schema = StateSchema(num_uints=1, num_byte_slices=2)
        local_schema = StateSchema(num_uints=0, num_byte_slices=0)

        # Tạo giao dịch ApplicationCreateTxn
        ...
        print(f"Confirmed transaction: {confirmed_txn}")

        # Lấy app_id từ giao dịch đã xác nhận
        app_id = confirmed_txn.get('application-index')

        if app_id is None:
            print("Error: Application was not created successfully")
        else:
            print(f"DAO contract deployed with Application ID: {app_id}")
    ...
    def get_transaction_history(self, account_address, limit=10):
        indexer_client = indexer.IndexerClient("", "https://testnet-api.algonode.cloud")
        try:
